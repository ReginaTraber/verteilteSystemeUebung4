...
public class IntegratorMain {
	...
	protected void setupEndpoints(String[] pArgs) {
			...
			mConvertedMsgRecordsIn = new MsgInboundEndpoint(String.join("_", CONV_WRECS, System.getProperty("user.name")), MSGDEST.QUEUE, MSGTYPE.WRECNORM);
			mAvgHourOut = new MsgOutboundEndpoint(avghourout != null ? avghourout : String.join("_", AVG_HOUR_DEST, System.getProperty("user.name")), MSGDEST.QUEUE, MSGTYPE.WAVGMSG);
			mAvgDayOut = new MsgOutboundEndpoint(avgdayout != null ? avgdayout : String.join("_", AVG_DAY_DEST, System.getProperty("user.name")), MSGDEST.QUEUE, MSGTYPE.WAVGMSG);
	        mAlarmOut = new MsgOutboundEndpoint(alarm != null ? alarm : String.join("_", ALARM_DEST, System.getProperty("user.name")), MSGDEST.QUEUE, MSGTYPE.WALARM);
	        mConvertedMsgRecordsOut = new MsgOutboundEndpoint(String.join("_", CONV_WRECS, System.getProperty("user.name")), MSGDEST.QUEUE, MSGTYPE.WRECNORM);
...
//        Uncomment next line for exercise ...
//        mFlows.add(new ProcessSequence(mConvertedMsgRecordsIn, mConsoleLogger));
    }
		...
		try {
		    ...
		    for (IProcessor flow : mFlows) {
		        try {
		            ...
		            System.err.printf("Exception while starting process: flow=%s ex=%s", (activeFlow != null) ? activeFlow.getClass().getSimpleName() : "unknown", pEx.getMessage());
	...
	private void run() {
	    ...
	    while (true) {
	        ...
	        try {
	            ...
	            System.err.printf("Exception while running process: flow=%s ex=%s", (activeFlow != null) ? activeFlow.getClass().getSimpleName() : "unknown", pEx.getMessage());
	            pEx.printStackTrace(System.err);
	        }
	        try {
